...
class Play
  ...
  def self.find_by_playwright(name)
    playwright = PlayDBConnection.find_by_name(name)

    raise "#{name} not found in Data base" unless playwright
...
class Playwright
  ...
  def self.all
    ...
    date.map {|datum| Playwright.new(datum)}
  end
  ...
  def get_plays
    raise "#{self} not in databse" unless self.id
